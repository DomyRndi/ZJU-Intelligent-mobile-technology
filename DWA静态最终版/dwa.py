...
class Config(object):
    ...
    max_dyawrate = 5  # max角加速度np.radians(5000.0)
    ...
    def calculate_clearance_cost(self, pose, velocity, barriers, config):
        ...
        while time < config.predict_time:
            ...
            for i in range(int(barriers.size/2)-0):
                ...
                i += 1
            time += config.dt
        return 100000.0/math.pow(min/80.0, 5)
    ...
    def planning(self, pose, velocity, goal, barriers, config):
        ...
        for i in range(int(nPossiblevelocity)+1):
            for j in range(int(nPossiblew)+1):
                ...
                j += 1
                # print("cost_vel:", cost_vel)
                # print("cost_head:", cost_head)
                # print("cost_clearance:", cost_clearance)
                # print("best_velocity", best_velocity)
            i += 1
